...
const employeeData = [
    ...
    },
    {
        ...
        validate: idInput => {
            valid = /^#?([a-f0-9]{6}|[a-f0-9]{3})$/.test(idInput)
            if (valid) {
                return true;
            } else {
...
const promptEngineer = (employees) => {
    ...
    newPrompt = employeeData.concat({
        type: 'input',
        name: 'github',
        message: 'Please include the github Link',
        validate: githubInput => {
            valid = /^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/.test(githubInput)
            if (valid) {
                return true;
            } else {
                console.log('Github URL is required!');
                return false;
            }
        }
    });
